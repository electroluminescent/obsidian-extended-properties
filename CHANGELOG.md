...
- **The interface speaks that name.** Every place the UI said "Type" now says your property's name - the no-type screen, the chip tooltip, the entry-options heading, the table title, settings headings and descriptions, import/export, macros, snapshots. Where the name lands mid-sentence it carries a slight accent tint.
