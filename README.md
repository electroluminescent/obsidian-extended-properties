...
- **Rating** *(deprecated)*: absorbed by the number type - enable *Rating icons* under a number's slider settings (any Obsidian icon, user-set maximum via the range Maximum, multi-row wrap, red fill for negatives). Existing rating properties keep rendering.
- **Link**: An internal `[[note]]` reference rendered as a clickable link, with `[[`-triggered note autocomplete while editing.
- **Unit** *(deprecated)*: absorbed by the number type - a number carries a unit suffix and an optional display factor (stored canonically, shown multiplied). Existing unit properties keep rendering.
...
Colour a property by its own value. Palettes are made once in **Settings -> Palettes** and pointed at from a property's *Formatting* block, so the sidebar, the type table and inline chips agree on what a value looks like. A palette also carries the finishes its values wear, and a property can borrow a palette's colours while writing its own points and bands - in values, or in per cent of that property's own range, so one scale fits a skill out of twenty and a fortune out of ten thousand. Three ways of reading a value: a **wheel** sweeping a hue across the property's range, a **scale** of points and bands - a point is one value wearing one colour, a band holds a colour flat between two edges, and either can be added anywhere on it (with linked edges, a tick to say which side wins a shared edge, and your choice of what happens in the gaps and beyond the ends) - and **words** with colours of their own. The colours are a list of their own beside the scale, so a colour can be moved without moving the numbers. Every blend travels through OKLCH, so red to blue passes through the purples rather than through grey, and a fill takes a foreground picked by measured contrast.
...
Over the top go **finishes** - matte, sheen, mirror, foil, spectra, prism, opal, nebula, beacon, glitter, crackle, satin, weave, relief, hammered - applied to everything, to named values, or to a band of numbers. Each is a material rather than a picture of one: the light follows your pointer, and every value is lit from where the pointer stands on IT - so a sheet of rows catches the light one row at a time as you cross them, and fades behind you. It travels at a speed with a ceiling on it and eases back to rest when the pointer leaves. All of it is gradients and blend modes; nothing is downloaded, nothing is an image, and nothing runs on a timer. The light also wanders slowly around the pointer, so a still hand still breathes. Reduced motion keeps the materials, stops the wandering and drops the speed limit to a crawl; Windows High Contrast stands the whole layer down.
