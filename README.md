...
Over the top go **finishes** - matte, gloss, foil, prismatic, holographic, iridescent, satin, emboss, sparkle, linen, crystal, radiant, hammered - applied to everything, to named values, or to a band of numbers. Each is gradients and a blend mode of its own, so it modulates the colour underneath rather than covering it; nothing is downloaded, nothing is an image, and nothing moves. The whole layer stands down under Windows High Contrast.
