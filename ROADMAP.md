...
### Cross-version durability (v3.0.0) [x]
...
### Dice presentation & polish (v3.3.0-v3.5.5) [x]
...
### Milestone 12 - Foundations: measure, test, finish the index [x] (shipped in v3.14.0)
...
#### N2 - Performance benchmark & regression harness [x] (S)

- **What.** A reproducible large-vault benchmark: a synthetic fixture
  generator (N notes x M types, cross-note references, aggregates) plus timed
...
#### F6 - Integration tests over the Obsidian seam [x] (M)
...
#### N1 - Aggregate memoization & large-vault scale [x] (M; optional worker deferred)
...
#### M1 - Accessibility completion [x] (shipped in v3.6.0)
...
- **What.** Re-approach the anti-aliasing that shipped in v3.5.0 and was
  locked off in v3.5.2: the 2x supersample distorted the dice under CSS 3D.
...
- **What.** Submit to `obsidianmd/obsidian-releases` per the runbook in
  [RELEASING.md](RELEASING.md) section 4: public repo, published (non-draft) release
